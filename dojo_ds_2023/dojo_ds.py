...
show_how_to_delete=False,print_names=False):
	...
	from .utils import show_del_me_code
	import sys
	import inspect
	import pandas as pd
	from IPython.display import display
...
def check_package_versions(packages = ['matplotlib','seaborn','pandas','numpy','sklearn','dojo_ds'],
	...
	import pandas as pd
	import inspect
	from .standard_imports import global_imports
...
def get_downloaded_file_paths(folder="Downloads", fname_query='/**/Part*',
	...
	"""
	import os,glob
	from dojo_ds.utils import inspect_file
	if folder=="Downloads":
		## Run the cell below to attempt to programmatically find your crime file
	...
	file_info = [inspect_file(f,verbose=False) for f in found_files]
	found_files = sorted(file_info, key=lambda x: x['Created'], reverse=True)
